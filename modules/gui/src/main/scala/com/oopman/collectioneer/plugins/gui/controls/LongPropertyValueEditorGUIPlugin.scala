...
class LongPropertyValueEditorGUIPlugin(stage: Stage @Id("com.oopman.collectioneer.plugins.GUIPlugin.stage"))
extends GUIPlugin(stage), PropertyValueEditorGUIPlugin:
  def canEditPropertyValuesForProperty(property: Property): Boolean =
    property.propertyTypes.contains(PropertyType.Long)
  ...
  def generatePropertyValueEditor(property: Property, propertyValue: Option[PropertyValue]): PropertyValueEditorGUIPlugin.PropertyValueEditor =
    new LongPropertyValueEditorGUIPlugin.LongPropertyValueEditor(property, propertyValue)

  def getName: String = "Long PropertyValue Editor GUI Plugin"

  def getVersion: String = "0.0.1"

  override def getRank: Int = -1

private object LongPropertyValueEditorGUIPlugin:
  private class LongPropertyValueEditor(property: Property, propertyValue: Option[PropertyValue])
    ...
    def getPropertyValue: Option[PropertyValue] =
      val values =
        for
          ...
          longValue <- value.toLongOption
        yield longValue
      Some(entity.projected.PropertyValue(
        longValues = values.toSeq
      ))
    ...
    override def generateInputNode: TextField =
      ...
      validator.createCheck()
        ...
        .withMethod { context =>
          ...
          try value.toLong
          catch case _: NumberFormatException => context.error("Invalid 64-bit integer value")
        }
        .decorates(textField)
        .immediate()
      textField