...
trait ScalikePropertyValueDAO:
  def getPropertyValuesByCollectionUUIDs(collectionUUIDs: Seq[UUID], propertyUUIDs: Seq[UUID] = Nil, deleted: Seq[Boolean] = Seq(false))(implicit session: DBSession): List[PropertyValue]
