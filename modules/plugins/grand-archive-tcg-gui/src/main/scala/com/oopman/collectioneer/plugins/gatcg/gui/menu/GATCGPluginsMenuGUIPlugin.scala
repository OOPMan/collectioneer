...
extends GUIPlugin(stage), PluginsMenuGUIPlugin:
  private lazy val settingsMenuItem = new MenuItem("Setttings"):
    onAction = event => {
      // TODO: Handle click
    }
  ...
  private lazy val downloadDatasetMenuItem = new MenuItem("Download Dataset"):
    onAction = event => {
       ...
       * Step 2: Show new window that downloads the dataset to the chosen file, displaying progress
       */
    }
  ...
  def getMenu: Menu = new Menu("Grand Archive TCG"):
    items = settingsMenuItem :: downloadDatasetMenuItem :: importDatasetMenuItem :: downloadImagesMenuItem :: Nil
