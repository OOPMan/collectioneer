...
object PropertyCollectionQueries:
  ...
  def upsert =
    sql"""
          ...
          SET index = excluded.index, property__collection_relationship_type = cast(excluded.property__collection_relationship_type AS property__collection__relationship_type) ,
          modified = now()
       """
