  ...
  extension (card: Models.Card)
    def asCollection: Collection = Collection(
      ...
      propertyValues = Map (
        ...
        CardProperties.effect -> PropertyValue (textValues = card.effect.map (_:: Nil).getOrElse (Nil)),
        CardProperties.effectRaw -> PropertyValue (textValues = card.effect_raw.map (_:: Nil).getOrElse (Nil)),
