  ...
  extension (innerCard: Models.InnerCard)
    def asCollection: Collection = Collection(
      ...
      propertyValues = Map(
        ...
        CardProperties.effect -> PropertyValue (textValues = innerCard.effect.map (_:: Nil).getOrElse (Nil)),
        CardProperties.effectRaw -> PropertyValue (textValues = innerCard.effect_raw.map (_:: Nil).getOrElse (Nil)),
